...
class TweetScraper(CrawlSpider):
    ...
    def start_requests(self):
        for j in range(len(self.limit)-1):
            for i in emojis:
                ...
                yield http.Request(tmpurl,headers=[("User-Agent", user_agent)],meta={'tmpurl':tmpurl,'emoji': i,"proxy": SETTINGS['PROXY']},callback=self.parse_tweet_page)
    ...
    def parse_tweet_page(self, response):
        ...
        except:yield http.Request(response.url,headers=[("User-Agent", user_agent)], meta={'tmpurl':response.meta['tmpurl'],'emoji': emoji,"proxy": SETTINGS['PROXY']},callback=self.parse_tweet_page)
        ...
        yield http.Request(url,headers=[("User-Agent", user_agent)], meta={'tmpurl':response.meta['tmpurl'],'emoji': emoji,"proxy": SETTINGS['PROXY']},callback=self.parse_tweet_page)
